...
// const SUPPLIER_PRICES = MATERIALS.reduce((acc, mat) => {
//   acc[mat.name] = mat.cost;
//   return acc;
// }, {});
...
//   let suitableMaterials = filteredMaterials.filter(m => SUPPLIER_PRICES[m.name] <= parsedBudget);
...
//   let bestMaterial = suitableMaterials.sort((a, b) => b.durability - a.durability || a.cost - b.cost)[0];

//   res.json({
//     recommended_material: bestMaterial.name,
//     cost: SUPPLIER_PRICES[bestMaterial.name],
...
//     Math.min(...Object.values(SUPPLIER_PRICES[m.name])) <= parsedBudget
//   );
...
//   // Sorting based on **durability only** (since priority is removed)
//   budgetMaterials.sort((a, b) => b.durability - a.durability || a.cost - b.cost);

//   let bestMaterial = budgetMaterials[0];

//   // Find the cheapest supplier price (but don't return supplier details)
